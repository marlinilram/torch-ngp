...
def get_camera_parameter_bin():
	from colmap_read_model import read_cameras_binary
	camdata = read_cameras_binary(os.path.join(TEXT_FOLDER, "sparse/0/cameras.bin"))
	list_of_keys = list(camdata.keys())
	cam = camdata[list_of_keys[0]]
	h, w, f = float(cam.height), float(cam.width), cam.params[0]
	fl_x = fl_y = f
	k1 = 0.0
	k2 = 0.0
	p1 = 0.0
...
def generate_transforms_from_bin(cam_data):
	from colmap_read_model import read_images_binary
	imdata = read_images_binary(os.path.join(TEXT_FOLDER, "sparse/0/images.bin"))
	# 不要用train.txt, NerfingMVS跑完以后会过滤train.txt中不好的帧
	with open(os.path.join(TEXT_FOLDER, 'valid_train.txt'), 'r') as f_list:
		lines = f_list.readlines()
		lines = [l.split('\n')[0] for l in lines]
	if os.path.exists(os.path.join(TEXT_FOLDER, 'train_ignore.txt')):
		ignore = json.load(open(os.path.join(TEXT_FOLDER, 'train_ignore.txt')))
		ignore = ignore['train_index']
		ignore = [lines[i] for i in ignore]
	else:
		ignore = []

    # reorder imdata to match train.txt
	name_im_id_dct = {im.name: im_id for im_id, im in imdata.items()}
	...
	centroid=np.zeros(3)
	up=np.zeros(3)
	# for im_id, im in imdata.items():
	for name in lines:
		if name not in name_im_id_dct:
			continue
		im_id = name_im_id_dct[name]
		im = imdata[im_id]
		if im.name in ignore:
			continue
		...
		name = str(f"./{image_rel}/{im.name}")
		b=sharpness(name)
		name = str(f"images/{im.name}")
		print(name, "sharpness=",b)
		image_id = im.id
		qvec = im.qvec
		tvec = im.tvec
		R = qvec2rotmat(-qvec)  # qvec正负不影响此处rotmat
		t = tvec.reshape([3,1])
	...
	return out, centroid, up


def generate_transforms_from_txt(cam_data):
...
if __name__ == "__main__":
	...
	cam_data = get_camera_parameter_bin()
	out, centroid, up = generate_transforms_from_bin(cam_data)
